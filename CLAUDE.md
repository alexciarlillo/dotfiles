...
- Skills are split across two source trees: `universal/.agents/skills/` (agent-agnostic) and `rblx/.agents/skills/` (Roblox-specific: `automated-reviewer`, `pr-review-doc`, `investigate-grafana-anomaly`, `voice-server-docs`). `bootstrap.sh`'s `agents_dots` stows both into `~/.agents/` and `~/.claude/skills/`, guarded on `rblx/.agents/skills/` existing. `AGENTS.md` as a file symlink, and each `skills/<skill>` as a **directory symlink** (see the folding note under Claude Code). **`.agents/AGENTS.md` is the one home for agent-agnostic standing instructions** — Codex reads it natively; Claude Code can't, so `universal/.claude/CLAUDE.md` imports it (below). Add standing instructions there, never per-agent. Note: `~/.agents` (dotted, agent config) is distinct from `~/agents` (no dot, the Unison-synced work-doc workspace / `AGENT_WORK_DIR`).
...
`universal/.agents/skills/` holds the agent-agnostic suite that moves work from idea → shippable PR, writing living docs to a cross-project workspace **outside any repo** under `~/agents/` (work management only, never committed). Roblox-specific skills (`automated-reviewer`, `pr-review-doc`, `investigate-grafana-anomaly`, `voice-server-docs`) live in `rblx/.agents/skills/` and are only stowed when the `rblx/` directory exists:
